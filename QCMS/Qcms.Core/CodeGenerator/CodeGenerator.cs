...
using Qcms.Core.DbHelper;

namespace Qcms.Core.CodeGenerator
{
    /// <summary>
    ...
    public class CodeGenerator
    {
        ...
        public CodeGenerator(IOptions<CodeGenerateOption> options)
        {
            ...
            if (_options.ConnectionString.IsNullOrWhiteSpace())
                throw new ArgumentNullException("未指定数据库连接串");
        ...
        public void GenerateTemplateCodesFromDatabase(bool isCoveredExsited = true)
        {
            ...
            if (tables != null && tables.Any())
            {
                ...
                {  
                    ...
                    GenerateDtos(table, isCoveredExsited);
        ...
        /// <param name="table">表名</param>
        /// <param name="isCoveredExsited">是否覆盖</param>
        private void GenerateDtos(DbTable table, bool isCoveredExsited = true)
        {
            ...
            var iDotsPath = _options.OutputPath + Delimiter + "Dtos";
            if (!Directory.Exists(iDotsPath))
            {
                Directory.CreateDirectory(iDotsPath);
            }
            var fullPath = iDotsPath + Delimiter + table.TableName + "Dtos.cs";
            if (File.Exists(fullPath) && !isCoveredExsited)
                return;

            var content = ReadTemplate("DtosTemplate.txt");
            content = content.Replace("{GeneratorTime}", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"))
                .Replace("{DtosNamespace}", _options.DtosNamespace)
                .Replace("{Author}", _options.Author)
                .Replace("{Comment}", table.TableComment)
                .Replace("{ModelName}", table.TableName)
                .Replace("{ModelProperties}", sb.ToString());
        ...
        {
            var iServicesPath = _options.OutputPath + Delimiter + "IService";
        ...
        /// <param name="keyTypeName"></param>
        /// <param name="ifExsitedCovered"></param>
        private void GenerateController(DbTable table, bool ifExsitedCovered = true)
        {
            var iServicesPath = _options.OutputPath + Delimiter + "Controller";
            if (!Directory.Exists(iServicesPath))
            {
                Directory.CreateDirectory(iServicesPath);
            }
            var fullPath = iServicesPath + Delimiter +  table.TableName + "Controller.cs";
            if (File.Exists(fullPath) && !ifExsitedCovered)
                return;
            ...
            content = content.Replace("{Comment}", table.TableComment)
                .Replace("{Author}", _options.Author)
                .Replace("{GeneratorTime}", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"))
                .Replace("{ControllerNamespace}", _options.ControllerNamespace)
                .Replace("{ModelName}", table.TableName);
            WriteAndSave(fullPath, content);
        }
        ...
        {
            var repositoryPath = _options.OutputPath + Delimiter + "Service";
        ...
        private string ReadTemplate(string templateName)
        {
            ...
            string path = $"{currentAssembly.GetName().Name}.CodeGenerator.Templates.{templateName}";
